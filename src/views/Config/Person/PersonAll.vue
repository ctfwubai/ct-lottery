...
function addPerson() {
  // 验证必填字段
  if (!newPerson.value.uid.trim() || !newPerson.value.name.trim()) {
    alert('工号和姓名为必填项')
    return
  }
  
  // 检查工号是否已存在
  const exists = allPersonList.value.some(person => person.uid === newPerson.value.uid.trim())
  if (exists) {
    alert('该工号已存在')
    return
  }
  ...
  // 创建新人员对象
  const personData = addOtherInfo([{
    uid: newPerson.value.uid.trim(),
    name: newPerson.value.name.trim(),
    department: newPerson.value.department.trim() || '未分配',
    identity: newPerson.value.identity.trim() || '员工',
    avatar: newPerson.value.avatar.trim() || ''
  }])
  
  if (personData.length > 0) {
    personConfig.addNotPersonList(personData)
    closeAddDialog()
  }
}
...
<template>
  ...
  <!-- 手工添加人员对话框 -->
  <dialog ref="addDialogRef" class="border-none modal">
    <div class="modal-box">
    ...
    <div class="flex gap-3">
      ...
      <div class="tooltip tooltip-bottom" :data-tip="t('tooltip.downloadTemplateTip')">
        <a
          ...
          :href="`/ct-lottery/${t('data.xlsxName')}`"
