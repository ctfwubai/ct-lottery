...
function selectPrize(item: IPrizeConfig) {
  ...
  selectedPrize.value.isUsedCount = item.isUsedCount
  selectedPrize.value.isUsed = item.isUsed

  // 同步奖项状态到手机控制端
  mobileControlStore.sendStatus()

  // 确保separateCount对象包含singleDrawCount
  if (!selectedPrize.value.separateCount) {
      ...
      countList: [
        {
          id: '0',
          count: item.count,
          isUsedCount: item.isUsedCount,
        },
      ],
    }
  }

  // 确保有singleDrawCount字段
  if (!selectedPrize.value.separateCount.singleDrawCount) {
    selectedPrize.value.separateCount.singleDrawCount = 1
  }

  // 如果countList为空或只有一个，初始化为默认值
  if (!selectedPrize.value.separateCount.countList || selectedPrize.value.separateCount.countList.length === 0) {
    selectedPrize.value.separateCount.countList = [
      {
        id: '0',
        count: item.count,
        isUsedCount: item.isUsedCount,
