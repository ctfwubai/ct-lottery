import type { IImage, IMusic, IGlobalDrawConfig, IFont } from '@/types/storeType'
...
export const useGlobalConfig = defineStore('global', {
    ...
    return {
      globalConfig: {
        ...
        drawConfig: {
          ...
          autoSwitchNextPrize: false, // 默认不自动切换，抽完奖项后需要手动切换
        } as IGlobalDrawConfig,
      },
      currentMusic: {
  ...
  getters: {
    ...
    // 获取抽奖配置
    getDrawConfig(state) {
      return state.globalConfig.drawConfig
    },
    // 获取字体列表
    getFontList(state) {
      return state.globalConfig.fontList
    },
    // 获取主标题配置
    getTitleConfig(state) {
      return state.globalConfig.theme.titleConfig
    },
  },
  actions: {
    // 设置rowCount
    ...
    // 设置抽奖配置
    setDrawConfig(drawConfig: IGlobalDrawConfig) {
      this.globalConfig.drawConfig = drawConfig
    },
    // 添加字体
    addFont(font: IFont) {
      for (let i = 0; i < this.globalConfig.fontList.length; i++) {
        if (this.globalConfig.fontList[i].name === font.name) {
          return
        }
      }
      this.globalConfig.fontList.push(font)
    },
    // 删除字体
    removeFont(fontId: string) {
      for (let i = 0; i < this.globalConfig.fontList.length; i++) {
        if (this.globalConfig.fontList[i].id === fontId) {
          this.globalConfig.fontList.splice(i, 1)
          break
        }
      }
    },
    // 重置字体列表
    resetFontList() {
      this.globalConfig.fontList = [] as IFont[]
    },
    // 设置主标题字体大小
    setTitleFontSize(fontSize: number) {
      this.globalConfig.theme.titleConfig.fontSize = fontSize
    },
    // 设置主标题字体
    setTitleFontFamily(fontFamily: string, fontUrl: string = '') {
      this.globalConfig.theme.titleConfig.fontFamily = fontFamily
      this.globalConfig.theme.titleConfig.fontUrl = fontUrl
    },
    // 重置所有配置
    reset() {
      this.globalConfig = {
